...
use lib\ViewHelper\HtmlHelper;
 ...
 <style>
    ...
    <br><br>
	<form method="post" action="<?= (isset($formation))? lib\App::$_WEBROOT_LINK. "admin/formation/" . $formation['id'] : lib\App::$_WEBROOT_LINK. "admin/formation"; ?>">
			...
			<div class="col-sm-4">
				<div class="form-group">
					...
					<select class='form-control' id='online' name='online'>
							<option value="0" <?= $formation['online'] == 0 ? " selected" : "" ?>>Brouillon</option>
							<option value="1" <?= $formation['online'] == 1 ? "selected" : "" ?>>Publié</option>
					</select>
				</div>
					...
					<select class='form-control' id='classification' name='classification'>
						...
						<?php foreach ($arr_categories as $k => $v): ?>
							...
							<?php if (isset($formation)): ?>
								<option value="<?= $v['id']; ?>" <?= $formation['id_category'] == $v['id'] ? "selected" : "" ?>><?= $v['name']; ?></option>
							<?php else: ?>
